...
entitydetails.pincode, entitydetails.contact, entitydetails.latitude, entitydetails.longitude, concat('http://localhost/Project-Sanjeevani/viewstockpharma.php?edid=', entitydetails.edid, '&productname=', '$prod_id') as URL from entitydetails WHERE fktype='2' and 
edid in (select phar_id from stock join products on products.prod_id = stock.prod_id where 
products.prod_genericname='$prod_id') and latitude <= ($latitude*$Range) AND latitude >= ($latitude/$Range) AND 
...
//echo "Product Name";
		 ...
		 echo "<input list ='productlist' name='prod_id' id='prod_id' class='form-control' required=''>";
